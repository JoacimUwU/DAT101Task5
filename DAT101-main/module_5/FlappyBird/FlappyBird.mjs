...
export function playSound(aSound) {
  if (!GameProps.soundMuted) {
    aSound.pause();          // Stop any current playback
    aSound.currentTime = 0;  // Rewind to start
    aSound.play();           // Play it again
  }
}
  ...
  switch (GameProps.status) {
    ...
    case EGameStatus.gameOver:
      ...
      for (let i = 0; i < GameProps.baits.length; i++) {
        ...
        // If the hero is close enough to a bait (within 15 pixels),
        // play a "food" sound and remove the bait from the array
        // Also gives the player 10 points
        if (dist < 15) {
          delBaitIndex = i;
          // Food sound
          playSound(GameProps.sounds.food);
          console.log("🍽️ Flappy ate the bait!");
        }
      }
      if (delBaitIndex >= 0) {
...
function spawnObstacle() {
  ...
  // Make sure new obstacles match current day/night setting
  obstacle.setDayNight(GameProps.dayTime);
...
// If a is true, use b, else use c
// a ? b : c;
function setDayNight() {
  // ---------------- TASK 6: Day and Night Mode ----------------
// Switch between day and night using radio buttons.
// When toggled, the background sprite and all existing obstacles
// update to match the current mode (day = bright, night = dark).
  GameProps.dayTime = rbDayNight[0].checked;
  GameProps.background.index = GameProps.dayTime ? 0 : 1;

  for (const obstacle of GameProps.obstacles) {
    obstacle.setDayNight(GameProps.dayTime);
  }

  console.log(GameProps.dayTime ? "🌞 Switched to Day" : "🌙 Switched to Night");
}

// ---------------- TASK 3: The Flap of Wings ----------------
// When the spacebar is pressed and the hero is not dead,
// the bird flaps upward and a "flap" sound is played.
